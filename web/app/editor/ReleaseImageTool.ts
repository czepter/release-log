...
export default class ReleaseImageTool {
  static get toolbox() {
    return { title: 'Bild', icon: ICON.image }
  }
  ...
  private paint(): void {
    ...
    if (this.src) {
      ...
      const alt = el('input', 'w-full border-0 border-b border-dashed border-zinc-200 bg-transparent px-0.5 py-1 text-[13px] text-zinc-600 outline-none focus:border-zinc-900', {
        placeholder: 'Alt-Text: was auf dem Bild zu sehen ist', 'aria-label': 'Alt-Text',
      })
      alt.value = this.alt
      alt.addEventListener('input', () => { this.alt = alt.value })
      const change = el('button', 'self-start text-xs text-zinc-500 underline underline-offset-2 cursor-pointer', { type: 'button' }, ['Anderes Bild wählen'])
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        ...
        status.textContent = err instanceof Error ? err.message : 'Hochladen fehlgeschlagen.'
      }
    })
    const uploadLabel = el('label', 'inline-flex h-8 cursor-pointer items-center rounded-md border border-zinc-200 bg-white px-3 text-[13px] font-medium hover:bg-zinc-50', {}, ['Neues Bild hochladen', input])
    this.root.append(el('div', 'flex flex-col gap-3 rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-4', {}, [
      el('span', 'text-sm font-medium', {}, ['Release-Bild']),
      ...(this.config.media().length ? [grid] : [el('span', 'text-xs text-zinc-500', {}, ['Dieses Log hat noch keine Bilder.'])]),
