...
useHead({
  title: () => m.value.landing.title,
  meta: () => [{ name: 'description', content: m.value.landing.description }],
})
...
const cta = computed(() => (session.value
  ? { href: '/dashboard', label: m.value.landing.ctaDashboardLong, short: m.value.landing.ctaDashboardShort }
  : { href: '/auth/github/login', label: m.value.common.signIn, short: m.value.landing.ctaSignInShort }))
...
<template>
  <div class="flex min-h-screen flex-col">
    <header class="border-b bg-card">
      <div class="mx-auto flex h-16 max-w-6xl items-center justify-between gap-6 px-4 sm:h-[72px] sm:px-6">
        ...
        <nav class="hidden items-center gap-8 text-sm text-muted-foreground md:flex">
          <a href="#funktionen" class="hover:text-foreground">{{ m.landing.navFeatures }}</a>
          <a href="#ablauf" class="hover:text-foreground">{{ m.landing.navHow }}</a>
          <a v-if="showcase" href="#changelog" class="hover:text-foreground">{{ m.landing.navChangelog }}</a>
          <a href="#faq" class="hover:text-foreground">{{ m.landing.navFaq }}</a>
        </nav>
        <AppLanguageSwitch />
      ...
      <div class="mx-auto grid max-w-6xl items-center gap-12 px-4 pt-14 pb-16 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:pt-28 lg:pb-30">
        <div class="flex flex-col items-start gap-5 lg:gap-6">
          <span class="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[13px] text-muted-foreground">
            <span class="size-1.5 rounded-full bg-green-500" />{{ m.landing.badge }}
          </span>
          <h1 class="text-[38px] leading-[42px] font-semibold tracking-tight sm:text-6xl sm:leading-[64px]">{{ m.landing.heroHeadline }}</h1>
          <p class="max-w-[540px] text-base leading-7 text-muted-foreground sm:text-lg sm:leading-[30px]">{{ m.landing.heroText }}</p>
          <div class="flex w-full flex-col gap-2.5 pt-1 sm:w-auto sm:flex-row sm:gap-3">
            ...
            <Button v-if="showcase" as="a" href="#changelog" variant="outline" size="lg" class="h-12 px-6">{{ m.landing.heroChangelogLink }} <ArrowRight class="size-4" /></Button>
          </div>
        </div>

        <div class="overflow-hidden rounded-2xl border bg-background shadow-[0_24px_48px_-24px_rgba(24,24,27,0.18)]" aria-hidden="true">
          <div class="flex h-11 items-center gap-2 border-b bg-card px-4 text-[13px] text-muted-foreground"><Plug class="size-3.5" />{{ m.landing.mockClient }}</div>
          ...
          <div class="mx-5 mb-5 flex flex-col gap-3 rounded-xl border bg-card p-5">
            ...
            <div class="text-lg leading-[26px] font-semibold tracking-tight">{{ m.landing.mockHeadline }}</div>
            <span class="self-start rounded-md px-2 py-0.5 text-xs font-medium" :class="SECTION_TONE.new">{{ m.sections.new }} · 3</span>
          </div>
        </div>
      </div>
    </section>
    ...
    <section id="problem">
      <div class="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-16 sm:px-6 lg:gap-14 lg:py-26">
        <div class="flex max-w-[760px] flex-col gap-3">
          <span class="text-xs font-medium tracking-[0.08em] text-muted-foreground uppercase sm:text-[13px]">{{ m.landing.problemEyebrow }}</span>
          <h2 class="text-[30px] leading-9 font-semibold tracking-tight sm:text-[40px] sm:leading-[48px]">{{ m.landing.problemHeadline }}</h2>
        </div>
        <div class="grid gap-6 md:grid-cols-3 md:gap-12">
          <div v-for="pain in m.landing.pains" :key="pain.title" class="flex flex-col gap-1.5 md:gap-2.5">
        ...
        <div class="grid items-stretch gap-3 lg:grid-cols-[minmax(0,1fr)_56px_minmax(0,1fr)] lg:gap-0">
          <div class="flex min-w-0 flex-col gap-4 rounded-2xl border bg-card p-5 lg:p-7">
            <span class="text-[13px] font-medium text-muted-foreground">{{ m.landing.gitLogSince }}</span>
          ...
          <div class="flex min-w-0 flex-col gap-3.5 rounded-2xl border bg-card p-5 lg:p-7">
            <span class="flex items-center gap-2.5">
              <span class="rounded-md bg-primary px-2 py-0.5 font-mono text-xs font-medium text-primary-foreground">v0.3.0</span>
              <span class="text-[13px] font-medium text-muted-foreground">{{ m.landing.releaseNotes }}</span>
            </span>
            <span class="text-lg leading-[26px] font-semibold tracking-tight sm:text-xl sm:leading-7">{{ m.landing.mockHeadline }}</span>
            <span class="self-start rounded-md px-2 py-0.5 text-xs font-medium" :class="SECTION_TONE.new">{{ m.sections.new }}</span>
            <ul class="flex flex-col gap-2.5">
              <li v-for="note in m.landing.notes" :key="note" class="grid grid-cols-[16px_minmax(0,1fr)] gap-1.5 text-sm leading-[22px] sm:text-[15px] sm:leading-6">
                <span class="mt-2.5 size-[5px] rounded-full bg-zinc-400" /><span>{{ note }}</span>
              </li>
            </ul>
            <p class="mt-1 text-[13px] leading-5 text-muted-foreground">{{ m.landing.notesFootnote }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section id="funktionen" class="border-y bg-card">
      <div class="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-16 sm:px-6 lg:gap-14 lg:py-26">
        <div class="flex max-w-[640px] flex-col gap-3">
          <span class="text-xs font-medium tracking-[0.08em] text-muted-foreground uppercase sm:text-[13px]">{{ m.landing.featuresEyebrow }}</span>
          <h2 class="text-[30px] leading-9 font-semibold tracking-tight sm:text-[40px] sm:leading-[48px]">{{ m.landing.featuresHeadline }}</h2>
        </div>
        <div class="grid gap-3 md:grid-cols-2 md:gap-5">
          <article class="flex flex-col gap-2 rounded-xl border bg-card p-5 md:gap-3 md:p-7">
            ...
            <h3 class="text-base font-semibold md:text-[17px]">{{ m.landing.featureRepoTitle }}</h3>
            <p class="text-[15px] leading-6 text-muted-foreground">{{ m.landing.featureRepoText }}</p>
          </article>
          <article class="flex flex-col gap-2 rounded-xl border bg-card p-5 md:gap-3 md:p-7">
            <span class="mb-1 hidden size-10 items-center justify-center rounded-lg bg-muted md:flex"><Plug class="size-5" /></span>
            <h3 class="text-base font-semibold md:text-[17px]">{{ m.landing.featureMcpTitle }}</h3>
            <p class="text-[15px] leading-6 text-muted-foreground">{{ m.landing.featureMcpText1 }} <code class="font-mono text-[13px]">write_release</code>{{ m.landing.featureMcpText2 }} <code class="font-mono text-[13px]">add_media</code>{{ m.landing.featureMcpText3 }}</p>
          </article>
          <article class="flex flex-col gap-2 rounded-xl border bg-card p-5 md:gap-3 md:p-7">
            <span class="mb-1 hidden size-10 items-center justify-center rounded-lg bg-muted md:flex"><Pencil class="size-5" /></span>
            <h3 class="text-base font-semibold md:text-[17px]">{{ m.landing.featureEditorTitle }}</h3>
            <p class="text-[15px] leading-6 text-muted-foreground">{{ m.landing.featureEditorText }}</p>
          </article>
          <article class="flex flex-col gap-2 rounded-xl border bg-card p-5 md:gap-3 md:p-7">
            <span class="mb-1 hidden size-10 items-center justify-center rounded-lg bg-muted md:flex"><Globe class="size-5" /></span>
            <h3 class="text-base font-semibold md:text-[17px]">{{ m.landing.featurePublicTitle }}</h3>
            <p class="text-[15px] leading-6 text-muted-foreground">{{ m.landing.featurePublicText1 }} <code class="font-mono text-[13px]">/l/&lt;id&gt;</code> {{ m.landing.featurePublicText2 }}</p>
          </article>
        </div>
      </div>
    </section>
    ...
    <section id="ablauf" class="border-b">
      <div class="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-16 sm:px-6 lg:gap-14 lg:py-26">
        <div class="flex max-w-[640px] flex-col gap-3">
          <span class="text-xs font-medium tracking-[0.08em] text-muted-foreground uppercase sm:text-[13px]">{{ m.landing.howEyebrow }}</span>
          <h2 class="text-[30px] leading-9 font-semibold tracking-tight sm:text-[40px] sm:leading-[48px]">{{ m.landing.howHeadline }}</h2>
        </div>
        <ol class="grid gap-7 md:grid-cols-3 md:gap-12">
          <li
            v-for="(step, i) in m.landing.steps" :key="step.title"
    ...
    <section v-if="showcase" id="changelog">
      <div class="mx-auto flex max-w-6xl flex-col gap-7 px-4 py-16 sm:px-6 lg:gap-14 lg:py-26">
        <div class="flex flex-col justify-between gap-6 md:flex-row md:items-end md:gap-12">
          <div class="flex max-w-[640px] flex-col gap-3">
            <span class="inline-flex items-center gap-2 text-xs font-medium tracking-[0.08em] text-muted-foreground uppercase sm:text-[13px]"><span class="size-1.5 rounded-full bg-green-500" />{{ m.landing.showcaseEyebrow }}</span>
            <h2 class="text-[30px] leading-9 font-semibold tracking-tight sm:text-[40px] sm:leading-[48px]">{{ t(m.landing.showcaseHeadline, { product: showcase.product }) }}</h2>
            <p class="text-[15px] leading-6 text-muted-foreground sm:text-[17px] sm:leading-7">
              {{ m.landing.showcaseTextHead }}<template v-if="showcase.older.length">{{ m.landing.showcaseTextOlder }}</template>.
            </p>
          </div>
          <Button as="a" :href="showcase.pageUrl" variant="outline" class="h-11 shrink-0 px-4">{{ m.landing.showcaseAll }} <ArrowRight class="size-4" /></Button>
        </div>
        ...
        <p class="-mt-3 font-mono text-xs text-muted-foreground lg:-mt-8">{{ m.landing.alsoJson }} <a :href="showcase.feedUrl" class="text-foreground/80 hover:underline">{{ showcase.feedUrl }}</a></p>
      </div>
    </section>
    ...
    <section id="faq" class="border-t bg-card">
      <div class="mx-auto grid max-w-6xl gap-6 px-4 py-16 sm:px-6 lg:grid-cols-[360px_minmax(0,1fr)] lg:gap-24 lg:py-26">
        <div class="flex flex-col gap-3">
          <span class="text-xs font-medium tracking-[0.08em] text-muted-foreground uppercase sm:text-[13px]">{{ m.landing.faqEyebrow }}</span>
          <h2 class="text-[30px] leading-9 font-semibold tracking-tight sm:text-[40px] sm:leading-[48px]">{{ m.landing.faqHeadline }}</h2>
          <p class="hidden text-[15px] leading-6 text-muted-foreground lg:block">{{ m.landing.faqIntro }}</p>
        </div>
        <div class="flex flex-col border-t">
          <details v-for="(faq, i) in m.landing.faqs" :key="faq.q" class="group border-b" :open="i === 0">
    ...
    <footer class="flex grow flex-col bg-zinc-950 text-zinc-50">
      <div class="mx-auto flex w-full max-w-6xl flex-col gap-5 px-4 pt-16 pb-14 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:gap-16 lg:pt-28 lg:pb-24">
        <div class="flex max-w-[720px] flex-col gap-4">
          <h2 class="text-[32px] leading-[38px] font-semibold tracking-tight sm:text-[44px] sm:leading-[52px]">{{ m.landing.footerHeadline }}</h2>
          <p class="text-base leading-[26px] text-zinc-400 sm:text-[17px] sm:leading-7">{{ session ? m.landing.footerTextSignedIn : m.landing.footerTextAnon }}</p>
        </div>
      ...
      <div class="mt-auto border-t border-zinc-800">
        <div class="mx-auto flex max-w-6xl flex-col gap-3.5 px-4 py-6 text-[13px] text-zinc-400 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-7">
          ...
          <nav class="flex flex-wrap gap-5 sm:gap-7">
            <a href="#funktionen" class="hover:text-zinc-50">{{ m.landing.navFeatures }}</a>
