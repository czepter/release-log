...
async function allow() {
  try {
    ...
    toast.success(`${newEntry.github_login} zugelassen.`)
    ...
    <Card class="gap-5 p-6">
      <div class="flex items-center gap-3.5">
        ...
        <div class="flex flex-col gap-1">
          ...
          <Badge :variant="session.isAdmin ? 'default' : 'secondary'">{{ session.isAdmin ? 'Admin' : 'Mitglied' }}</Badge>
        </div>
      </div>
      <dl class="flex flex-col gap-3 text-[13px]">
        ...
        <div class="flex justify-between gap-3"><dt class="text-muted-foreground">Zuletzt angemeldet</dt><dd>{{ when(session.lastSeenAt) }}</dd></div>
        <div class="flex justify-between gap-3"><dt class="text-muted-foreground">Verbundene Clients</dt><dd>{{ clients?.clients.length ?? 0 }}</dd></div>
      </dl>
      <Separator />
      <Button variant="outline" size="lg" @click="logout"><LogOut /> Abmelden</Button>
    </Card>

    <div class="flex flex-col gap-6">
      <section class="overflow-hidden rounded-xl border bg-card">
        <div class="flex flex-col gap-0.5 border-b px-5 py-4">
          <h2 class="text-base font-semibold">Verbundene Clients</h2>
          <p class="text-[13px] text-muted-foreground">Trennen widerruft alle Access- und Refresh-Token des Clients sofort.</p>
        </div>
        <p v-if="!clients?.clients.length" class="px-5 py-10 text-center text-sm text-muted-foreground">Keine verbundenen Clients.</p>
        <ul v-else>
          <li v-for="client in clients.clients" :key="client.clientId" class="flex items-center gap-3.5 border-b px-5 py-4 last:border-b-0">
            ...
            <Button variant="outline" size="sm" class="text-destructive" @click="revoke(client)">Trennen</Button>
          </li>
        </ul>
      </section>
      ...
      <section v-if="session.isAdmin" class="overflow-hidden rounded-xl border bg-card">
        <div class="flex flex-col gap-0.5 border-b px-5 py-4">
          <h2 class="text-base font-semibold">Zulassungsliste</h2>
          <p class="text-[13px] text-muted-foreground">Nur diese GitHub-Logins dürfen sich anmelden. Admins aus <code class="font-mono">ADMIN_LOGINS</code> immer.</p>
        </div>
        <form class="grid gap-2.5 border-b bg-muted/40 px-5 py-4 sm:grid-cols-[200px_minmax(0,1fr)_auto]" @submit.prevent="allow">
          <Input v-model="newEntry.github_login" aria-label="GitHub-Login" required placeholder="GitHub-Login" class="bg-background font-mono" />
          <Input v-model="newEntry.note" aria-label="Notiz" placeholder="Notiz (optional)" class="bg-background" />
          <Button type="submit">Zulassen</Button>
        </form>
        <p v-if="!allowlist?.entries.length" class="px-5 py-10 text-center text-sm text-muted-foreground">Noch niemand auf der Liste.</p>
        <table v-else class="w-full text-sm">
          <thead>
            <tr class="h-10 border-b text-left text-xs text-muted-foreground">
              <th class="pl-5 font-medium">Login</th><th class="font-medium">Notiz</th><th class="font-medium">Hinzugefügt</th><th />
            </tr>
          </thead>
          <tbody>
            <tr v-for="entry in allowlist.entries" :key="entry.githubLogin" class="h-14 border-b last:border-b-0">
              ...
              <td class="pr-4 text-right"><Button variant="ghost" size="sm" @click="remove(entry)">Entfernen</Button></td>
            </tr>
          </tbody>
        </table>
      </section>
